...
class MailerConfig 
{
    ...
    */
    public function envelope(array $value = []): \var\cache\dev\Symfony\Config\Framework\Mailer\EnvelopeConfig
    {
        if (null === $this->envelope) {
            $this->_usedProperties['envelope'] = true;
            $this->envelope = new \var\cache\dev\Symfony\Config\Framework\Mailer\EnvelopeConfig($value);
     ...
     * @template TValue
     * @param TValue $value
     * @return \var\cache\dev\Symfony\Config\Framework\Mailer\HeaderConfig|$this
     * @psalm-return (TValue is array ? \var\cache\dev\Symfony\Config\Framework\Mailer\HeaderConfig : static)
     */
    public function header(string $name, mixed $value = []): \var\cache\dev\Symfony\Config\Framework\Mailer\HeaderConfig|static
    {
        if (!\is_array($value)) {
        ...
        if (!isset($this->headers[$name]) || !$this->headers[$name] instanceof \var\cache\dev\Symfony\Config\Framework\Mailer\HeaderConfig) {
            $this->_usedProperties['headers'] = true;
            $this->headers[$name] = new \var\cache\dev\Symfony\Config\Framework\Mailer\HeaderConfig($value);
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('envelope', $value)) {
            $this->_usedProperties['envelope'] = true;
            $this->envelope = new \var\cache\dev\Symfony\Config\Framework\Mailer\EnvelopeConfig($value['envelope']);
            unset($value['envelope']);
        }

        if (array_key_exists('headers', $value)) {
            $this->_usedProperties['headers'] = true;
            $this->headers = array_map(function ($v) { return \is_array($v) ? new \var\cache\dev\Symfony\Config\Framework\Mailer\HeaderConfig($v) : $v; }, $value['headers']);
            unset($value['headers']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['headers'])) {
            $output['headers'] = array_map(function ($v) { return $v instanceof \var\cache\dev\Symfony\Config\Framework\Mailer\HeaderConfig ? $v->toArray() : $v; }, $this->headers);
        }
