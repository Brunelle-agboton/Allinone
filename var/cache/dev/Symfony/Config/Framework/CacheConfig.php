...
class CacheConfig 
{
     ...
     * @template TValue
     * @param TValue $value
     * @return \var\cache\dev\Symfony\Config\Framework\Cache\PoolConfig|$this
     * @psalm-return (TValue is array ? \var\cache\dev\Symfony\Config\Framework\Cache\PoolConfig : static)
     */
    public function pool(string $name, mixed $value = []): \var\cache\dev\Symfony\Config\Framework\Cache\PoolConfig|static
    {
        if (!\is_array($value)) {
        ...
        if (!isset($this->pools[$name]) || !$this->pools[$name] instanceof \var\cache\dev\Symfony\Config\Framework\Cache\PoolConfig) {
            $this->_usedProperties['pools'] = true;
            $this->pools[$name] = new \var\cache\dev\Symfony\Config\Framework\Cache\PoolConfig($value);
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('pools', $value)) {
            $this->_usedProperties['pools'] = true;
            $this->pools = array_map(function ($v) { return \is_array($v) ? new \var\cache\dev\Symfony\Config\Framework\Cache\PoolConfig($v) : $v; }, $value['pools']);
            unset($value['pools']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['pools'])) {
            $output['pools'] = array_map(function ($v) { return $v instanceof \var\cache\dev\Symfony\Config\Framework\Cache\PoolConfig ? $v->toArray() : $v; }, $this->pools);
        }
