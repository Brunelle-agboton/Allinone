...
class RateLimiterConfig 
{
    ...
    public function limiter(string $name, array $value = []): \var\cache\dev\Symfony\Config\Framework\RateLimiter\LimiterConfig
    {
        if (!isset($this->limiters[$name])) {
            $this->_usedProperties['limiters'] = true;
            $this->limiters[$name] = new \var\cache\dev\Symfony\Config\Framework\RateLimiter\LimiterConfig($value);
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('limiters', $value)) {
            $this->_usedProperties['limiters'] = true;
            $this->limiters = array_map(function ($v) { return new \var\cache\dev\Symfony\Config\Framework\RateLimiter\LimiterConfig($v); }, $value['limiters']);
