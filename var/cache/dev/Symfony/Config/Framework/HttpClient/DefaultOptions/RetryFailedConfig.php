...
namespace var\cache\dev\Symfony\Config\Framework\HttpClient\DefaultOptions;
...
class RetryFailedConfig 
{
     ...
     * @return \var\cache\dev\Symfony\Config\Framework\HttpClient\DefaultOptions\RetryFailed\HttpCodeConfig|$this
     * @psalm-return (TValue is array ? \var\cache\dev\Symfony\Config\Framework\HttpClient\DefaultOptions\RetryFailed\HttpCodeConfig : static)
     */
    public function httpCode(string $code, array $value = []): \var\cache\dev\Symfony\Config\Framework\HttpClient\DefaultOptions\RetryFailed\HttpCodeConfig|static
    {
        if (!\is_array($value)) {
        ...
        if (!isset($this->httpCodes[$code]) || !$this->httpCodes[$code] instanceof \var\cache\dev\Symfony\Config\Framework\HttpClient\DefaultOptions\RetryFailed\HttpCodeConfig) {
            $this->_usedProperties['httpCodes'] = true;
            $this->httpCodes[$code] = new \var\cache\dev\Symfony\Config\Framework\HttpClient\DefaultOptions\RetryFailed\HttpCodeConfig($value);
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('http_codes', $value)) {
            $this->_usedProperties['httpCodes'] = true;
            $this->httpCodes = array_map(function ($v) { return \is_array($v) ? new \var\cache\dev\Symfony\Config\Framework\HttpClient\DefaultOptions\RetryFailed\HttpCodeConfig($v) : $v; }, $value['http_codes']);
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['httpCodes'])) {
            $output['http_codes'] = array_map(function ($v) { return $v instanceof \var\cache\dev\Symfony\Config\Framework\HttpClient\DefaultOptions\RetryFailed\HttpCodeConfig ? $v->toArray() : $v; }, $this->httpCodes);
        }
